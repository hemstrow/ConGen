...
![Monarch Butterfy (*Danaus plexippus*), (c) William Hemstrom 2022](./accessory_files/monarch_photo.jpg){width="454"}
...
![Expansion of monarch butterflies across the Pacific (Hemstrom et al in revision)](./accessory_files/monarch_map.jpg){width="454"}
...
![Monarch migratory pathways in North America](./accessory_files/monarch_map_NAM.png){width="454"}
...
monarch_vcf <- read_vcf("monarchs/monarchs.vcf", sample.meta = "monarchs/monarch_metadata.tsv")
monarch_data <- format_snps(monarch_vcf, output = "adegenet", facets = "pop")
...
```{r, include = T, echo = F}
cluster <- find.clusters(monarch_data, max.n.clust = 20, n.pc = 170, stat = "BIC")

#Assign individuals to the groups determined in the command above
monarch_data$pop <- cluster$grp

#Run the DAPC on the assignments from "find.clusters"
monarch_data.dapc <- dapc(monarch_data, n.pca = 170, n.da = 2)

#Plot the results in PCA-space
scatter(monarch_data.dapc, scree.da = TRUE, scree.pca = TRUE, posi.pca = "bottomright", posi.da = "bottomleft", clab = 0)

#Produce a plot of individual membership to each of the clusters
compoplot(monarch_data.dapc, lab = "", txt.leg = seq(levels(monarch_data.dapc$assign)))
...
```{r, include = T, echo = F}
opt.pca <- optim.a.score(monarch_data.dapc)

monarch_data.dapc2 <- dapc(monarch_data, n.pca = opt.pca$best, n.da=2)
scatter(monarch_data.dapc2, scree.da = TRUE, scree.pca = TRUE, posi.pca = "topleft", posi.da = "bottomleft", clab = 0)

compoplot(monarch_data.dapc2, lab = "", txt.leg = seq(levels(monarch_data.dapc$assign)))
